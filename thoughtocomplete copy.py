...
def llm(query):
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Please complete the user's text. Do not repeat the user's text. Do not add or say anything else. Simply respond by 'autocompleting' their text. If they've completed a sentence, continue with the next sentence.They will be asking about nursey rhymes.",
            },
...
def gpt(completion):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a completion tool. All you do is complete the users thoughts."},
